...
static const FormatInfo format_map[] = {
    ...
    // swizzled
    { SDL_PIXELFORMAT_ABGR8888, NV097_SET_TEXTURE_FORMAT_COLOR_SZ_A8B8G8R8, true, false, "SZ_A8B8G8R8" },
    { SDL_PIXELFORMAT_RGBA8888, NV097_SET_TEXTURE_FORMAT_COLOR_SZ_R8G8B8A8, true, false, "SZ_R8G8B8A8" },
    { SDL_PIXELFORMAT_ARGB8888, NV097_SET_TEXTURE_FORMAT_COLOR_SZ_A8R8G8B8, true, false, "SZ_A8R8G8B8" },
    { SDL_PIXELFORMAT_ARGB8888, NV097_SET_TEXTURE_FORMAT_COLOR_SZ_X8R8G8B8, true, false, "SZ_X8R8G8B8" },
    { SDL_PIXELFORMAT_BGRA8888, NV097_SET_TEXTURE_FORMAT_COLOR_SZ_B8G8R8A8, true, false, "SZ_B8G8R8A8" },
    { SDL_PIXELFORMAT_RGB565, NV097_SET_TEXTURE_FORMAT_COLOR_SZ_R5G6B5, true, false, "SZ_R5G6B5" },
    { SDL_PIXELFORMAT_ARGB1555, NV097_SET_TEXTURE_FORMAT_COLOR_SZ_A1R5G5B5, true, false, "SZ_A1R5G5B5" },
    { SDL_PIXELFORMAT_ARGB1555, NV097_SET_TEXTURE_FORMAT_COLOR_SZ_X1R5G5B5, true, false, "SZ_X1R5G5B5" },
    { SDL_PIXELFORMAT_ARGB4444, NV097_SET_TEXTURE_FORMAT_COLOR_SZ_A4R4G4B4, true, false, "SZ_A4R4G4B4" }, 
    ...
    // linear yuv color space
    { SDL_PIXELFORMAT_RGBA8888, NV097_SET_TEXTURE_FORMAT_COLOR_LC_IMAGE_CR8YB8CB8YA8, false, true, "YUY2" },
    //{ SDL_PIXELFORMAT_RGBA8888, NV097_SET_TEXTURE_FORMAT_COLOR_LC_IMAGE_YB8CR8YA8CB8, false, true, "UYVY" },
...
bool update_texture_memory(SDL_PixelFormatEnum format, int width, int height, bool swizzled)
{
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            pixels[y * width + x] = SDL_MapRGB(gradient_surf->format, xScale, yScale, xScale);
        }
    }

    //if (SDL_UnlockSurface(gradient_surf))
    //    return false;
    ...
    // TODO: actually width/height that need to be broken out better for non-square textures
    for (int i = 0; i < num_vertices; i++) {
        float val = swizzled ? 1.0f : 256.0f;
...
int main(void)
{
    ...
    init_shader();
    update_texture_memory(format_map[format_map_index].SdlFormat, 256, 256, format_map[format_map_index].XboxSwizzled);
    ...
    while(1) {
        ...
        */
        ...
        p = pb_push1(p, NV097_SET_TEXTURE_OFFSET + 0*64, (uint32_t)texture.addr & 0x03ffffff);
        if (format_map[format_map_index].XboxSwizzled) {
            p = pb_push1(p, NV097_SET_TEXTURE_FORMAT + 0*64,
                ...
                MASK(NV097_SET_TEXTURE_FORMAT_BASE_SIZE_U, bsf(texture.width)) |
                MASK(NV097_SET_TEXTURE_FORMAT_BASE_SIZE_V, bsf(texture.height)) |
            ...
            p = pb_push1(p, NV097_SET_TEXTURE_FORMAT + 0*64,
                ...
                MASK(NV097_SET_TEXTURE_FORMAT_COLOR, format_map[format_map_index].XboxFormat) |
                MASK(NV097_SET_TEXTURE_FORMAT_MIPMAP_LEVELS, 1) |
                MASK(NV097_SET_TEXTURE_FORMAT_BASE_SIZE_U, 0) |
                MASK(NV097_SET_TEXTURE_FORMAT_BASE_SIZE_V, 0) |
                MASK(NV097_SET_TEXTURE_FORMAT_BASE_SIZE_P, 0));
